  ...
  methods: {
    ...
    go () {
      ...
      var add = '?Customer_name=' + this.Customer_name + '&Customer_linkman=' + this.Customer_linkman + '&Customer_connect=' + this.Customer_connect +
          '&Customer_stylist=' + this.Customer_stylist + '&Customer_referrer=' + this.Customer_referrer +
      ...
      this.axios.get('https://formattingclub.com/YiNuoLogin/AfterSale/AddCustomer' + add).then(res => {
        if (res.data === '录入成功') {
          mui.alert('录入成功', function () {
            _this.$router.push('money_sale')
          })
        }else{
          mui.alert('录入失败 提示：有可能项目名重复')
        }
      })
    }
  }
